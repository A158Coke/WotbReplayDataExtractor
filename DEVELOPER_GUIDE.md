...
- Java 离线版：纯离线、双击运行、自动打开浏览器、可选择/拖拽回放、预览并导出 xlsx。
...
仓库按"语言/形态"分层：`common/`(共享资源) + `common/python/`(车辆库更新脚本) + `java/`(Java 主线 wotb-core + wotb-web) + `offline/`、`online/`(两种运行方式的 compose 入口)。
...
> 离线版与服务器版功能完全一致（回放提取 + 排行榜上传查询），区别仅在于数据存本地 PostgreSQL 容器、镜像从 Docker Hub 拉取而非源码编译。
...
- 实时 rating 展示列：`rating`、`kast`、`contribution`、`influence`、`damage_avg`、`potential_damage_avg`、`potential_damage_supplement_avg`、`assist_avg`、`multi_damage_rate`、`kills`、`kills_avg`；`average_hp` 和 `account_id` 不再展示。
...
- `KAST`：参考 CS2/CS:GO 的 Kill / Assist / Survive / Traded 思路，单场取 `damage/(average_hp*1.1)`、`assist/(average_hp*1.25)`、`win && survived ? 1 : 0`、`traded_death ? 1 : 0`、`(damage+assist)/(average_hp*1.15)` 的最大值，再跨场平均。
- `influence`：统计全部场次，按 `damage + assist` 在双方总池中的占比（期望份额 `1/14`）和人头折算成影响力指数。
...
- `rating`：`potential-DPB`、KAST、影响力、AST（`assist_avg`）、多伤率、场均人头加权得到，权重为 30% / 20% / 25% / 10% / 10% / 5%，最终乘以 10 输出近似 1000 分量级。
### 评分（Rating）
...
- 离线版 采用本地 Web UI，已处理端口占用、浏览器自动启动、程序退出服务残留等问题。
...
- 小改动也要考虑 Java 离线版 与 Web 版是否需要同步。
